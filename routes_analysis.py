...
def _value_label (spec ,v ):
    ...
    value_SI =spec ["to_si"](v )


    t_ac ,t_dc =_ref_times (param_name ,value_SI )


    t_dc =_dc_time_guard (param_name ,value_SI ,t_dc )


    dc_label =f"DC {t_dc :.0f}"if t_dc is not None else "DC —"

    return f"{spec ['fmt'](v )}  (AC {t_ac :.0f} / {dc_label } min)"
...
def _dc_time_guard (param_name :str ,value_SI :float ,T_DC ):
    """Czas DC tylko w obrębie sufitu DC; powyżej zwraca None (etykieta 'DC —').
    Wyrównuje legendę do krzywych, które DC powyżej sufitu już pomijają."""
    ceil =DC_CEILING_SI .get (param_name )
    if ceil is not None and value_SI >ceil +1e-6 :
        return None 
    return T_DC 
...
def plot_wariant_velocity (param_name ,save_dir =OUTPUT_DIR ,decim =50 ):
    ...
    values =spec ["values"]
    colors =_colors (values )
    v_max_base =max (ROUTE_VMAX_KMH .values ())
...
SWEEP_RANGES_ROUTE ={
"v_max":{
"values":[v /3.6 for v in range (250 ,401 ,10 )],
...
"P_nom":{
"values":[6e6 +1e6 *i for i in range (7 )],
