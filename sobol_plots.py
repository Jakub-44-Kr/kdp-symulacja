...
# Kolory: AC i DC kontrastowe (do porównania)
COLOR_AC = "#1f77b4"  # niebieski
COLOR_DC = "#d62728"  # czerwony
COLOR_S1 = "#4c72b0"  # ciemny niebieski (samodzielny)
COLOR_ST = "#dd8452"  # pomarańczowy (całkowity)
...
# ═══════════════════════════════════════════════════════════════════════════
#  WYKRES 1: Bar chart S_i vs S_T_i (osobno dla każdego systemu)
# ═══════════════════════════════════════════════════════════════════════════
...
def _draw_heatmap(
    ...
    if triangular:
        # Maska dolnego trójkąta (zostawiamy tylko górny, włącznie z diagonalą)
        mask = np.tril(np.ones_like(matrix, dtype=bool), k=-1)
        matrix_display = np.where(mask, np.nan, matrix)
    else:
        matrix_display = matrix

    # Skala kolorów - max z absolutnej wartości
    ...
    for i in range(n):
        for j in range(n):
            ...
            if triangular and i > j:
                continue
            if i == j:
                # Diagonala: przekreślona (S2 nie ma sensu dla i=j w macierzy interakcji)
...
def plot_top_interactions(
    ...
    # Wyciągnij wszystkie unikalne pary (i, j) z i < j
    pairs = []
    for i in range(n):
    ...
    # Sortuj malejąco po wartości (absolutnej)
